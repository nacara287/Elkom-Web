...
{
    public class SENETController : Controller
    {
        ...
        public void changestring()
        {
            ...
            else
            {
                if (Session["LoginAltUser"] != null)
                {
                    ...
                    userid = alt.ID;
                    if (alt.görme.Contains("Senetler"))
                    {
                        gorme = true;
                        ...
                        if (alt.ekleme.Contains("Senetler"))
                            ekleme = true;
                        if (alt.silme.Contains("Senetler"))
                            silme = true;
                        if (alt.degistir.Contains("Senetler"))
                            degistirme = true;
        ...
        public class carifismodelhareket
        {
            ...
            {
                SenetÖdeme, SenetTahsil, KarşılıksızSenet, Senetİade, CiroluSenetİade, FirmaSenetİade
        ...
        public JsonResult GetHareketData(string search, string order, int offset, int limit, string sort, int id, filtermodel[] filterler)
        {
            ...
            var v = (from a in db.SENETLER_ALINAN_HAREKET.Where(x => x.deleted != 1).Where(x => DbFunctions.TruncateTime(x.TARIH) >= DbFunctions.TruncateTime(zaman) && DbFunctions.TruncateTime(x.TARIH) <= DbFunctions.TruncateTime(sonzaman)).Where(query) select a);
            ...
            foreach (var item in data)
            {
                ...
                yenihareket.SENETDURUMU = item.SENETDURUMU.ToString();
        ...
        public JsonResult GetHareketVerilenData(string search, string order, int offset, int limit, string sort, int id, filtermodel[] filterler)
        {
            ...
            var v = (from a in db.SENETLER_VERILEN_HAREKET.Where(x => x.deleted != 1).Where(x => DbFunctions.TruncateTime(x.TARIH) >= DbFunctions.TruncateTime(zaman) && DbFunctions.TruncateTime(x.TARIH) <= DbFunctions.TruncateTime(sonzaman)).Where(query) select a);
        ...
        public JsonResult VERILENFISDATA(string search, string order, int offset, int limit, string sort, filtermodel[] filterler)
        {
            ...
            var v = (from a in db.SENETLER_VERILEN.Select(pr => new
            {
                hareket = pr,
            ...
            }).Where(x => x.hareket.deleted != 1).Where(x => DbFunctions.TruncateTime(x.hareket.VADE.Value) >= DbFunctions.TruncateTime(zaman) && DbFunctions.TruncateTime(x.hareket.VADE.Value) <= DbFunctions.TruncateTime(sonzaman)).Where(query) select a);
            ...
            foreach (var item in data)
            {
                ...
                yenihareket.KESIDETARIHI = item.hareket.KESIDETARIHI.Value.ToShortDateString();
                ...
                yenihareket.SENETDURUMU = item.hareket.SENETDURUMU.ToString();
        ...
        public JsonResult ALINANFISDATA(string search, string order, int offset, int limit, string sort, filtermodel[] filterler)
        {
            ...
            var v = (from a in db.SENETLER_ALINAN.Select(pr => new
            {
                hareket = pr,
                ...
                VERILENKISIADI = pr.CIROKISI != null ? pr.CIROKISI.FIRMAUNVANI : "",
                VERILENKISIKODU = pr.CIROKISI != null ? pr.CIROKISI.CARIKOD : "",
                S_MERKEZI = pr.S_MERKEZI.ACIKLAMA
            }).Where(x => x.hareket.deleted != 1).Where(x => DbFunctions.TruncateTime(x.hareket.VADE.Value) >= DbFunctions.TruncateTime(zaman) && DbFunctions.TruncateTime(x.hareket.VADE.Value) <= DbFunctions.TruncateTime(sonzaman)).Where(query) select a);
            ...
            foreach (var item in data)
            {
                ...
                yenihareket.VADE = item.hareket.VADE.Value.ToString("yyyy-MM-dd");
                yenihareket.KESIDETARIHI = item.hareket.SENETKESIDETARIHI.Value.ToShortDateString();
                ...
                if (!string.IsNullOrEmpty(item.hareket.SENETADRES1))
                    yenihareket.CEKKESIDEYERI = item.hareket.SENETADRES1;
                yenihareket.TUTAR = item.hareket.TUTAR.ToString();
                ...
                try
                {
                    ...
                    foreach (var item in gelenveri.gelenhareket)
                    {
                        if ((int)item.HAREKET == 1 || (int)item.HAREKET == 2)
                        {
                            ...
                            {
                                case 1: alınancek.SENETDURUMU = CEKLER_ALINAN.alinandurumlar.TahsilEdildi; break;
                                case 2: alınancek.SENETDURUMU = CEKLER_ALINAN.alinandurumlar.Karşılıksız; break;
                                case 3: alınancek.SENETDURUMU = CEKLER_ALINAN.alinandurumlar.İadeEdilenler; break;
                            ...
                            if ((int)item.HAREKET == 1)
                            {
                                ...
                                if ((int)item.HAREKET == 0)
                                    kasahareket.ISLEMTURU = KASA_HAREKET.kasaislemtürü.Senet;
                                if ((int)item.HAREKET == 1)
                                    kasahareket.ISLEMTURU = KASA_HAREKET.kasaislemtürü.TSenet;
                                if ((int)item.HAREKET == 2)
                                {

                                    kasahareket.ISLEMTURU = KASA_HAREKET.kasaislemtürü.TKarşılıksızSenet;
                                }
                                ...
                                if ((int)item.HAREKET == 0)
                                    kasahareket2.ISLEMTURU = KASA_HAREKET.kasaislemtürü.Senet;
                                if ((int)item.HAREKET == 1)
                                    kasahareket2.ISLEMTURU = KASA_HAREKET.kasaislemtürü.TSenet;
                                if ((int)item.HAREKET == 2)
                                {

                                    kasahareket2.ISLEMTURU = KASA_HAREKET.kasaislemtürü.TKarşılıksızSenet;
                                }
                        ...
                        if ((int)item.HAREKET == 0)
                        {
                            var alınancek = db.SENETLER_VERILEN.Where(x => x.PORTFOYNO == item.PORTFOYNO).First();
                            switch ((int)item.HAREKET)
                            {
                                case 0: alınancek.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.Ödenmiş; break;
                                case 4: alınancek.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.İadeAlınanlar; break;
                                case 5: alınancek.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.İadeAlınanlar; break;

                            }
                            alınancek.ODEMETARIHI = DateTime.Parse(gelenveri.TARIH);
                            db.Entry(alınancek).State = EntityState.Modified;
                            var cekhareket = new SENETLER_VERILEN_HAREKET();
                            switch ((int)item.HAREKET)
                            {
                                case 0: cekhareket.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.Ödenmiş; break;
                                case 4: cekhareket.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.İadeAlınanlar; break;
                                case 5: cekhareket.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.İadeAlınanlar; break;
                            }
                            cekhareket.SENETID = alınancek.ID;

                            cekhareket.ACIKLAMA = item.ACIKLAMA;
                            cekhareket.DOSYANO = gelenveri.FISNO;
                            cekhareket.ACIKLAMA = item.PORTFOYNO;
                            cekhareket.TARIH = DateTime.Now;
                            ...
                            cekhareket.CreateDate = DateTime.Now;
                            cekhareket.CreateUserID = userid;
                            db.SENETLER_VERILEN_HAREKET.Add(cekhareket);
                            if ((int)item.HAREKET == 0)
                            {
                        ...
                        if ((int)item.HAREKET == 3 || (int)item.HAREKET == 4 || (int)item.HAREKET == 5)
                        {
                            ...
                            {
                                var alınancek = db.SENETLER_ALINAN.Where(x => x.PORTFOYNO == item.PORTFOYNO).First();
                                alınancek.SENETDURUMU = CEKLER_ALINAN.alinandurumlar.İadeEdilenler;

                                alınancek.VADE = DateTime.Parse(gelenveri.TARIH);
                                db.Entry(alınancek).State = EntityState.Modified;
                                db.SaveChanges();
                                var cekhareket = new SENETLER_ALINAN_HAREKET();
                                cekhareket.SENETDURUMU = CEKLER_ALINAN.alinandurumlar.İadeEdilenler;
                            ...
                            {
                                var alınancek = db.SENETLER_VERILEN.Where(x => x.PORTFOYNO == item.PORTFOYNO).First();
                                alınancek.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.İadeAlınanlar;

                                alınancek.ODEMETARIHI = DateTime.Parse(gelenveri.TARIH);
                                db.Entry(alınancek).State = EntityState.Modified;
                                db.SaveChanges();
                                var cekhareket = new SENETLER_VERILEN_HAREKET();
                                cekhareket.SENETDURUMU = CEKLER_VERILEN.verilendurumlar.İadeAlınanlar;

                                cekhareket.SENETID = alınancek.ID;

                                cekhareket.ACIKLAMA = item.PORTFOYNO +"/"+item.ACIKLAMA;
                                cekhareket.DOSYANO = gelenveri.FISNO;
                            ...
                            hareket.ISLEMTURU = CARI_HAREKET.tür.Senetİade;
        ...
        // GET: CEK
        public ActionResult VerilenSenetler()
        {
            changestring();
        ...
        public ActionResult SenetIslemleri()
        {
            changestring();
        ...
        public ActionResult SenetHareketleri(int id)
        {
            changestring();
            ...
            model.kodu = db.SENETLER_ALINAN.Where(x => x.ID == id).FirstOrDefault().PORTFOYNO;
        ...
        public ActionResult SenetHareketleriVerilen(int id)
        {
            changestring();
            ...
            model.kodu = db.SENETLER_VERILEN.Where(x => x.ID == id).FirstOrDefault().PORTFOYNO;
